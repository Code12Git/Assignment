...
const WorkinProgressCard = ({ workplan, number }) => {
  const label = { inputProps: { "aria-label": "Checkbox demo" } };

  return (
    <div className="max-w-2xl mx-auto  mt-40 md:mt-0  bg-white shadow-md hover:shadow-lg rounded-lg overflow-hidden transition-transform">
      <div className="p-4 md:p-6 w-full md:w-full flex flex-col cursor-pointer md:flex-row items-center gap-4 md:gap-6 justify-between">
        <div className="flex items-center space-x-5 md:space-x-24 flex-grow">
          ...
          <div className="flex flex-col space-y-1">
            <p className="text-lg font-semibold">{workplan.bookname}</p>
            <p className="text-sm text-gray-600">Book name</p>
          </div>
          <div className="flex flex-col space-y-1">
            <p className="text-lg font-semibold">{workplan.username}</p>
